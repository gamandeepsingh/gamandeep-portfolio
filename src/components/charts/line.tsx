...
export function Line({
  ...
  // Held in state (via callback ref) rather than a ref because the highlight
  // segment is derived from the path's geometry during render.
  const [pathEl, setPathEl] = useState<SVGPathElement | null>(null)
  const [pathLength, setPathLength] = useState(0)

  // Unique gradient ID for this line (stable across server and client renders)
  const instanceId = useId()
  const gradientId = `line-gradient-${dataKey}-${instanceId}`
  ...
  useEffect(() => {
    if (pathEl && animate) {
      ...
      if (len > 0) {
        // Measuring the rendered path is only possible after commit.
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setPathLength(len)
      }
    }
  }, [pathEl, animate, data, innerWidth])
  ...
  return (
    <>
      ...
      {showHighlight && isHovering && isLoaded && pathEl && (
        <motion.path
          animate={{ opacity: 1 }}
